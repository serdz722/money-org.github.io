...
<div class="col-md-8">								<div class="s-bk-lf">
									<div class="title">Список систем</div>
								</div>
								<div class="silver-bk"><br>
<BR />

<center><a href = "/?menu=tvorojok&sel=pay_systems" class="stn">Список систем</a> || <a href = "?menu=tvorojok&sel=pay_systems&add" class="stn">Добавить систему</a></center>
<BR />
<?PHP
if(isset($_POST["del"])){
		...
		if(strlen($title) >= 3){
			...
			if(strlen($comment) >= 30){
				...
				if($min_pay > 0){
					...
					$db->Query("INSERT INTO db_pay_systems (title, first_char, comment, min_pay) VALUES ('".mysql_real_escape_string($title)."','$first_char','$comment','$min_pay')");
...
<input type="text" name="title" size="45" value="<?=(isset($_POST["title"])) ? htmlspecialchars($_POST["title"]) : false; ?>" /><BR /><BR />
<b>Первая буква кошелька:</b><BR />
<input type="text" name="first_char" size="5" value="<?=(isset($_POST["first_char"])) ? htmlspecialchars($_POST["first_char"]) : false; ?>" /> (Например для WMZ первая буква Z)<BR /><BR />
<b>Минималка (<?=$config->VAL; ?>):</b><BR />
<input type="text" name="min_pay" size="5" value="<?=(isset($_POST["min_pay"])) ? htmlspecialchars($_POST["min_pay"]) : false; ?>" /> (Допускаются только целые числа)<BR /><BR />
<b>Комментарий (Например, описание платежки) (До 100 символов):</b><BR />
<textarea name="comment" cols="75" rows="25"><? echo htmlspecialchars($_POST["comment"]); ?></textarea><BR /><BR />
